          ...
          <div className={styles.dashboardGrid}>
            ...
            <div className={styles.sidebar}>
              ...
              <div className={styles.card}>
                ...
                <div className={styles.batterySliderTrack}>
                  <div className={styles.batterySliderFill} style={{ width: `${battery_level_pct}%` }} />
                </div>
                <div className={styles.batteryLabel} style={{ marginTop: '8px' }}>
                  <span>Arrival Battery</span>
                  <strong style={{ color: arrival_battery_pct < 15 ? '#DC2626' : '#16A34A' }}>{arrival_battery_pct}%</strong>
                </div>
                <div className={styles.batterySliderTrack} style={{ marginTop: '4px' }}>
                  <div className={styles.batterySliderFill} style={{ width: `${arrival_battery_pct}%`, background: arrival_battery_pct < 15 ? '#DC2626' : '#16A34A' }} />
                </div>
              </div>
            </div>
            ...
            <div className={styles.mainArea} style={{ display: 'block', textAlign: 'left', padding: 0, overflow: 'hidden', minHeight: 'auto' }}>
              <div className={styles.resultHeader}>
                <span>✨</span> AI-Optimized Route:{' '}
                {charging_stops.length === 0 ? 'No charging stops needed!' : `${charging_stops.length} charging stop${charging_stops.length !== 1 ? 's' : ''} selected`}
              </div>
              ...
              <div className={styles.resultBody}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '10px' }}>🗺️ Route Map</div>
                  <RouteMap result={result} height="380px" />
                  <div style={{ fontSize: '11px', color: '#9CA3AF', marginTop: '6px' }}>Click on markers to see details. Scroll to zoom.</div>
                </div>

                <div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '10px' }}>Route Overview</div>
                <div className={styles.timelineContainer}>
                  <div className={styles.timelineLine} />
                  <div className={styles.timelinePoint} title="Start" />
                  {charging_stops.map((_, i) => (
                    <div key={i} className={`${styles.timelinePoint} ${styles.stop}`} title={`Stop ${i + 1}`} />
                  ))}
                  <div className={`${styles.timelinePoint} ${styles.dest}`} title="End" />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#9CA3AF', marginBottom: '30px' }}>
                  ...
                  {charging_stops.map((s, i) => <span key={i}>{s.distance_from_start_km.toFixed(0)} km</span>)}
                  <span>Arrival ({arrival_battery_pct}%)</span>
                </div>

                <div className={styles.aiBox}>
                  <div style={{ fontSize: '20px' }}>🤖</div>
                  <div><strong>AI Recommendation</strong><br />{ai_recommendation}</div>
                </div>
                    ...
                    {charging_stops.map((stop, i) => (
                      <div key={i} className={styles.stopCard}>
                        ...
                        {stop.connection_types && stop.connection_types.length > 0 && (
                          <div style={{ marginTop: '8px', fontSize: '11px', color: '#6B7280' }}>🔌 {stop.connection_types.join(', ')}</div>
                        )}
                      </div>
                    ))}
                  </>
